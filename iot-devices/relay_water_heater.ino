// Relay Actuator Sketch for ESP32 - Water Heater Control
...
const char* mqtt_topic_command = "actuators/water_heater/1/command";  // Subscribe topic for commands
const char* mqtt_topic_status = "actuators/water_heater/1/status";    // Publish topic for status
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, mqtt_topic_command) == 0) {
    if (message == "on" || message == "1") {
      ...
      Serial.println("Water Heater turned ON");
    } else if (message == "off" || message == "0") {
      relayState = false;
      digitalWrite(RELAY_PIN, LOW);
      client.publish(mqtt_topic_status, "off");
      Serial.println("Water Heater turned OFF");
    }
  }
}
...
void reconnect() {
  while (!client.connected()) {
    ...
    String clientId = "ESP32-WATER_HEATER-";
    clientId += device_id;
    clientId += "-";
