// Relay Actuator Sketch for ESP32 - Curtains/Blinds Control
...
const char* mqtt_topic_command = "actuators/curtains_blinds/1/command";  // Subscribe topic for commands
const char* mqtt_topic_status = "actuators/curtains_blinds/1/status";    // Publish topic for status
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, mqtt_topic_command) == 0) {
    if (message == "on" || message == "1") {
      ...
      Serial.println("Curtains/Blinds turned ON");
    } else if (message == "off" || message == "0") {
      relayState = false;
      digitalWrite(RELAY_PIN, LOW);
      client.publish(mqtt_topic_status, "off");
      Serial.println("Curtains/Blinds turned OFF");
    }
  }
}
...
void reconnect() {
  while (!client.connected()) {
    ...
    String clientId = "ESP32-CURTAINS_BLINDS-";
    clientId += device_id;
    clientId += "-";
