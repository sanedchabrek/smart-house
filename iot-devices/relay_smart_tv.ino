// Relay Actuator Sketch for ESP32 - Smart TV Control
...
const char* mqtt_topic_command = "actuators/smart_tv/1/command";  // Subscribe topic for commands
const char* mqtt_topic_status = "actuators/smart_tv/1/status";    // Publish topic for status
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, mqtt_topic_command) == 0) {
    if (message == "on" || message == "1") {
      ...
      Serial.println("Smart TV turned ON");
    } else if (message == "off" || message == "0") {
      relayState = false;
      digitalWrite(RELAY_PIN, LOW);
      client.publish(mqtt_topic_status, "off");
      Serial.println("Smart TV turned OFF");
    }
  }
}
...
void reconnect() {
  while (!client.connected()) {
    ...
    String clientId = "ESP32-SMART_TV-";
    clientId += device_id;
    clientId += "-";
